...
FROM registry.knowdee.com/library/maven:3.8.6-jdk17 as mavenLayer
ARG PROJECT_NAME
COPY --from=gitLayer /data /data
WORKDIR /data
RUN mvn clean package &&  ls /data/$PROJECT_NAME/target/*.jar

FROM registry.knowdee.com/library/jmx-exporter:0.18.0-jdk17
ARG PROJECT_NAME
WORKDIR /data
COPY --from=mavenLayer /data/$PROJECT_NAME/target/*.jar /data/app.jar
#CMD echo "caoyong echo JAVA_OPTS:"$JAVA_OPTS
#docker build  --no-cache  -t registry.knowdee.com/tests/app:v1.0.0  . -f ./dockerfiles/java11.dockerfile \
