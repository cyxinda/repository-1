...
RUN mkdir -p /opt/jmx-exporter \
    &&  wget -O  /opt/jmx-exporter/simple-config.yml https://raw.githubusercontent.com/knowdee/repository/main/java/jmx/0.18.0/simple-config.yml \
    &&  wget -O  /opt/jmx-exporter/jmx_prometheus_javaagent-0.18.0.jar  https://raw.githubusercontent.com/knowdee/repository/main/java/jmx/0.18.0/jmx_prometheus_javaagent-0.18.0.jar

CMD mkdir -p /data/logs && echo $JAVA_OPTS && exec java $JAVA_OPTS  -jar /data/app.jar

# docker build  --no-cache  -t registry.knowdee.com/library/jmx-exporter:0.18.0-jdk11 . -f ./dockerfiles/jmx.dockerfile   --progress=plain
# docker run -v /tmp/apps:/data -p 18080:8080 -p 17070:7070 --rm --name app registry.knowdee.com/library/jmx-exporter:0.18.0-jdk11
